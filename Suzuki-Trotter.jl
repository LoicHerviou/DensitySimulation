...
"""
function joinDensityMatrix!(extendedRho, matRho, indexMin, indexMinTER, indexMax, indexMaxTER, Traces, TracesSmall)
    if indexMin==indexMinTER && indexMax==indexMaxTER
        ...
        for l in 1:2:length(matRho)-1
            joinDensityMatAux!(extendedRho[(l+1)÷2], matRho[l], matRho[l+1], Traces, TracesSmall)
        end
    ...
    elseif indexMin==indexMinTER+1 && indexMax==indexMaxTER
        ...
        for l in 2:2:length(matRho)-1
            joinDensityMatAux!(extendedRho[l÷2+1], matRho[l], matRho[l+1], Traces, TracesSmall)
        end
...
TracesSmall are the trace matrix for matRho
"""
function joinDensityMatAux!(rhoGuess, rhoLeft, rhoRight, Traces, TracesSmall)
...
function main(hx::Float64, hz::Float64, T::Float64; maxLen=3, dt=0.0001)
    ...
    for j in 1:length(tim)
        ...
        joinDensityMatrix!(extendedRho_o, matRho, minMatRho, indices_o[1], maxMatRho, indices_o[2], Traces, TracesSmall);   #Build the larger density Matrix
        ...
        joinDensityMatrix!(extendedRho_e, matRho, minMatRho, indices_e[1], maxMatRho, indices_e[2], Traces, TracesSmall)   #Build the larger density Matrix
